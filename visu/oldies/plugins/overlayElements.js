/*
	overlayElements plugins works with jQuery
	by tuurma
 */
(function($) {
	...
	/*
		Plugin declaration
	 */
	$.overlayElements = function(options) {
		var defaults = {
			overlayClass: '.reveal',
			wrapperEl: 'body',
			overlayIndex: 8,
		...
		plugin.start = function() {
			...
			var wel = $(plugin.settings.wrapperEl),
				el = wel.find(plugin.settings.overlayClass),
				welP = wel.position(),
				welW = wel.width(),
				welH = wel.height(),
			...
			if(over.length === 0){
				...
				$(wel).append('<div class="revealOverlay" data-revealwrapper="'+ plugin.settings.wrapperEl +'" data-revealclass="'+ plugin.settings.overlayClass +'" style="top:'+ welP.top +'px; left:'+ welP.left +'px; width:'+ welW +'px; height:'+ welH +'px; background-color: '+ plugin.settings.overlayColor +'; opacity: 0.8; position: absolute; z-index: '+ plugin.settings.overlayIndex +'"></div>');
		...
		var removeReveal = function(overlayEl) {
			...
			var revealWrapper = overlayEl.data('revealwrapper'),
				overlayClass = overlayEl.data('revealclass'),
				elWrapper = overlayEl.closest(revealWrapper),
				el = elWrapper.find(overlayClass);
